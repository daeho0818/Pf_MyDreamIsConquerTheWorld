...
cCityMob2::cCityMob2(Vec2 pos, vector<cBullet*>& bullets, float size)
	:cMob(pos, size), m_bullets(bullets)
{
	m_image = IMAGE->MakeVecImg("city_mob2");
			...
			t_Pattern1 = new cTimer(0.4, [&]()->void {
				...
				for (int i = -5; i <= 5; i++)
				{
					if (i != 0)
					{
						...
						m_bullets.push_back(new cReflexBullet(m_pos, dir, "bullet_city_mob2", "city_mob2_effect", "mob", m_damage, 0.5, 400, true));
					}
				}
				p1Count++;
				t_Pattern1 = nullptr;
