...
cChurchMob2::cChurchMob2(Vec2 pos, vector<cBullet*>& bullets, float size)
			...
			t_Pattern1 = new cTimer(0.4, [&]()->void {
				...
				for (int i = -5; i <= 5; i++)
				{
					if (i != 0)
					{
						...
						m_bullets.push_back(new cReflexBullet(m_pos, dir, "bullet_church_mob2", "church_mob2_effect","mob", m_damage, 0.5, 400, true));
					}
				}
				p1Count++;
				t_Pattern1 = nullptr;
